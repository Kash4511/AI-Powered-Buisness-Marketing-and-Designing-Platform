...
def generate_pdf_status(request, job_id):
    """
    Returns job status. If complete and pdf_url is a Cloudinary public_id
    rather than a full URL, regenerates a fresh signed URL on the fly.
    """
    ...
    # If the stored pdf_url is a raw public_id (not a https:// URL), sign it now
    if pdf_url and not pdf_url.startswith("http"):
        fresh_url = _get_signed_pdf_url(pdf_url)
        if fresh_url:
            job_data["pdf_url"] = fresh_url

    # If complete but pdf_url is still missing, try to get it from the LeadMagnet
        ...
        if lm_id:
            try:
                ...
                if lm.pdf_file:
                    ...
                    if fresh_url:
                        job_data["pdf_url"] = fresh_url
                        # Also persist it so next poll gets it
