...
const GlobalStyles = () => (
  <style>{`
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .fai-nav-item { display:flex; align-items:center; gap:9px; padding:9px 10px; border-radius:8px; font-size:0.82rem; font-weight:500; color:#666666; text-decoration:none; cursor:pointer; transition:all 0.15s; font-family:'Instrument Sans',sans-serif; }
    .fai-nav-item:hover, .fai-nav-item.active { background:#f7f7f5; color:#111111; }
    .fai-logout-btn { display:flex; align-items:center; gap:6px; background:none; border:1px solid rgba(0,0,0,0.08); border-radius:8px; padding:7px 14px; font-family:'Instrument Sans',sans-serif; font-size:0.78rem; font-weight:500; color:#666666; cursor:pointer; transition:all 0.2s; }
    .fai-logout-btn:hover { border-color:rgba(0,0,0,0.15); color:#111111; background:#f7f7f5; }
    .fai-sidebar-create { display:flex; align-items:center; justify-content:center; gap:8px; width:100%; padding:11px 14px; background:#0a0a0a; color:#fff; border:none; border-radius:10px; font-family:'Instrument Sans',sans-serif; font-size:0.82rem; font-weight:600; cursor:pointer; transition:all 0.2s; }
    .fai-sidebar-create:hover { background:#2a2a2a; transform:translateY(-1px); box-shadow:0 6px 20px rgba(0,0,0,0.15); }
    .fai-send-btn { display:inline-flex; align-items:center; gap:7px; padding:10px 18px; background:#0a0a0a; color:#fff; border:none; border-radius:9px; font-family:'Instrument Sans',sans-serif; font-size:0.82rem; font-weight:600; cursor:pointer; transition:all 0.2s; flex-shrink:0; }
    .fai-send-btn:hover:not(:disabled) { background:#2a2a2a; }
    .fai-send-btn:disabled { opacity:0.4; cursor:not-allowed; }
    .fai-attach-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 12px; background:#f7f7f5; border:1px solid rgba(0,0,0,0.08); border-radius:8px; font-family:'Instrument Sans',sans-serif; font-size:0.78rem; font-weight:500; color:#666666; cursor:pointer; transition:all 0.2s; }
    .fai-attach-btn:hover { border-color:rgba(0,0,0,0.15); color:#111111; }
    .fai-attach-btn.has-template { background:#0a0a0a; border-color:#0a0a0a; color:#fff; }
    .fai-chat-ta { width:100%; resize:none; border:none; outline:none; font-family:'Instrument Sans',sans-serif; font-size:0.88rem; color:#111111; background:transparent; line-height:1.6; min-height:24px; max-height:200px; overflow-y:auto; }
    .fai-chat-ta::placeholder { color:#aaaaaa; }
...
const FormaAI: React.FC = () => {
  ...
  const addMsg = (role: Message['role'], text: string, extra?: Partial<Message>) => {
    msgId.current += 1
    setMessages(prev => [...prev, { id: msgId.current, role, text, ...extra }])
  }
   ...
   * pollForPDF
   * ----------
   * Polls /api/pdf-generation/status/<jobId>/ until complete.
   * When complete, fetches the PDF via apiClient.get(..., { responseType: 'blob' })
   * so the Authorization header is sent automatically and the backend base URL
   * (https://django-msvx.onrender.com) is used — NOT localhost:5173.
  ...
  const pollForPDF = async (jobId: string, title: string, lmId?: number) => {
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      ...
      try {
        const statusRes = await apiClient.get(`/api/pdf-generation/status/${jobId}/`)
        ...
        // Still processing — update last system message with live progress
   ...
   * handleDownload
   * --------------
   * Uses apiClient so the Authorization header is always sent.
   * Never relies on a raw <a href="/api/..."> which would hit localhost:5173.
   */
  const handleDownload = async (msg: Message) => {
    try {
      if (msg.pdfUrl?.startsWith('blob:')) {
        // blob URL is already in memory — download directly
        const a = document.createElement('a')
        a.href     = msg.pdfUrl
        a.download = `${msg.pdfTitle ?? 'lead-magnet'}.pdf`
      ...
      // Re-fetch with auth
      const path = msg.lmId ? `/api/lead-magnets/${msg.lmId}/download/` : msg.pdfUrl
      if (!path) return
        ...
        <div style={{ background: '#fff', border: `1px solid ${T.bd}`, borderRadius: 14, overflow: 'hidden', maxWidth: 540 }}>
          ...
          <div style={{ padding: '14px 18px', borderBottom: `1px solid ${T.bd}`, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: "'Fraunces',serif", fontSize: '0.88rem', fontWeight: 700, color: T.dark, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {msg.pdfTitle}
              </div>
              <div style={{ fontSize: '0.68rem', color: T.t3, marginTop: 1 }}>PDF ready — preview below</div>
            </div>
            {/* Button calls handleDownload — never a raw href */}
            <button
              onClick={() => handleDownload(msg)}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: T.dark, color: '#fff', border: 'none', borderRadius: 8, fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer', flexShrink: 0, fontFamily: "'Instrument Sans',sans-serif" }}
            >
              <Download size={13}/> Download PDF
            </button>
          </div>
          {/* Iframe — blob: URL requires no auth */}
          <div style={{ height: 480, background: T.bg2 }}>
            {msg.pdfUrl ? (
              <iframe src={msg.pdfUrl} title={msg.pdfTitle} style={{ width: '100%', height: '100%', border: 'none' }}/>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: T.t3, fontSize: '0.82rem' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        {!isUser && (
          <div style={{ width: 28, height: 28, borderRadius: '50%', background: isError ? '#fff2f2' : T.dark, border: isError ? '1px solid rgba(200,50,50,0.2)' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginRight: 10, marginTop: 2 }}>
            {isError ? <span style={{ fontSize: 13 }}>!</span> : <Sparkles size={13} color="#fff"/>}
          </div>
        )}
        <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '228px 1fr', minHeight: 'calc(100vh - 62px)', fontFamily: "'Instrument Sans',sans-serif" }}>
        ...
        <aside style={{ background: '#fff', borderRight: `1px solid ${T.bd}`, padding: '28px 16px', display: 'flex', flexDirection: 'column', gap: 28, position: 'sticky', top: 62, height: 'calc(100vh - 62px)', overflowY: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '0 4px' }}>
            ...
            <div>
              <div style={{ fontFamily: "'Fraunces',serif", fontSize: '0.82rem', fontWeight: 700, color: T.t1, lineHeight: 1.2 }}>AI Lead Magnets</div>
              <div style={{ fontSize: '0.68rem', color: T.t3 }}>Your AI Workforce</div>
            </div>
          </div>
            ...
            <nav style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {navItems.map(item => (
                <a key={item.id} href={item.href}
                  className={`fai-nav-item${item.href === '/forma-ai' ? ' active' : ''}`}
          ...
          <div style={{ marginTop: 'auto' }}>
            ...
            <div
              style={{ background: selectedTemplateId ? T.dark : T.bg2, border: `1px solid ${selectedTemplateId ? T.dark : T.bd}`, borderRadius: 10, padding: '14px', cursor: selectedTemplateId ? 'default' : 'pointer', transition: 'all 0.2s' }}
            ...
            >
              ...
              {selectedTemplateId && (
                <button
                  onClick={e => { e.stopPropagation(); setSelectedTemplateId(''); setSelectedTemplateName(''); setArchitecturalImages([]) }}
        ...
        <main style={{ display: 'flex', flexDirection: 'column', background: T.bg2, height: 'calc(100vh - 62px)' }}>
          ...
          <div style={{ padding: '24px 36px 0', background: T.bg2 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: T.bg2, border: `1px solid ${T.bd}`, borderRadius: 20, padding: '4px 13px 4px 9px', fontSize: '0.7rem', fontWeight: 600, color: T.t2, marginBottom: 12 }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: T.dark }}/>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 36px 12px' }}>
            {messages.length === 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 16, paddingBottom: 40 }}>
                ...
                <div style={{ fontFamily: "'Fraunces',serif", fontSize: '1.1rem', fontWeight: 700, color: T.dark, textAlign: 'center' }}>
                  Describe your business
                </div>
                <div style={{ fontSize: '0.82rem', color: T.t3, textAlign: 'center', maxWidth: 320, lineHeight: 1.6 }}>
                  Tell us what you do, who you serve, and which type of lead magnet you want (Guide, Checklist, Trends Report, etc).
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', marginTop: 8 }}>
                  {[
                    'I run a perfume business selling 3, 5, 10ml bottles — create a Trends Report',
                    ...
                    <button key={p} onClick={() => setMessage(p)}
                      style={{ padding: '7px 14px', background: '#fff', border: `1px solid ${T.bd}`, borderRadius: 20, fontSize: '0.75rem', color: T.t2, cursor: 'pointer', fontFamily: "'Instrument Sans',sans-serif", transition: 'all 0.15s' }}
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div style={{ padding: '14px 16px' }}>
                <textarea
                  ...
                  placeholder="Describe your business and mention the type of lead magnet (e.g. Guide, Checklist, Trends Report)..."
                  rows={1}
              ...
              <div style={{ padding: '10px 14px', borderTop: `1px solid ${T.bd}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: T.bg2 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <button className={`fai-attach-btn${selectedTemplateId ? ' has-template' : ''}`} onClick={() => setShowTemplateModal(true)}>
                    <PdfIcon size={14}/>
                  ...
                  <span style={{ fontSize: '0.7rem', color: T.t3 }}>Press Enter to send</span>
                </div>
